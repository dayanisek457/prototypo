...
(count, family) => count + (family.variants ? family.variants.length : 0),
0,
);
}
...
data: {
allPresets: clone(
db.presets.filter(preset => preset.isPrototypoPreset !== false),
),
},
};
}
...
values: clone(db.user.appValues || {}),
	...
	if (query.includes('query getLibraryUserInfos') || query.includes('query {')) {
...
font => font.variant && font.variant.id === where.variant.id,
),
},
};
}
...
data: {
allAbstractedFonts: allAbstractedFonts.filter(
font => font.preset && font.preset.id === where.preset.id,
),
},
};
}
...
data: {
Variant: found
? {id: found.variant.id, values: clone(found.variant.values || {})}
: null,
},
};
}
...
db.user.library = db.user.library.filter(family => family.id !== variables.id);
saveDb(db);
...
family: {
id: (findVariant(db, variables.variantId) || {family: {id: null}}).family.id,
},
}
: null,
};
...
db.user.accessToken = {
...(db.user.accessToken || {id: variables.id, token: `local-token-${db.nextId}`}),
domains: variables.domainNames,
};
saveDb(db);
...
const uploads = JSON.parse(window.localStorage.getItem(LOCAL_UPLOAD_KEY) || '[]');
